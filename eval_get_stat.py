...
from utils.quant_utils import wrap_to_quant_model, init_weight_quantizer, init_input_quantizer, register_online_had, init_k_quantizer, init_v_quantizer,get_quantized_stat
...
def main():
    ...
    cal_dataloader, _ = get_loaders(
        "pile",
        tokenizer,
        train_size=4, # 1 for test, should be 64
        val_size=0,
        seed=2,
        seqlen=512,
    )
    prefixed_tokens = get_prefixed_tokens(cal_dataloader, model, tokenizer, "Llama-2-7b-hf", outlier_threshold=64, activation_type='down_proj')
    ...
    get_quantized_stat(model, cal_dataloader, 'max', prefixed_tokens)
